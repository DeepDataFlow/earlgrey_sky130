...
# NOTE: uniquify is intentionally skipped here. All sub-block instances of
# the same type are electrically identical (all #(...) parameter overrides
# were stripped from the wrapper source before stitching, since the real
# parameter values are already baked into each sub-block's own synthesized
# netlist), so sharing one module definition per sub-block type across all
# its instances is both valid and far cheaper than duplicating them.
