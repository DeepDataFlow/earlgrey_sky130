...
proc report_ports {} {
    ...
    foreach line [split $dump "\n"] {
        ...
        if {[regexp {^\s+wire\s+width\s+(\d+)\s+(input|output|inout)\s+\d+\s+\\(\S+)} $line -> width dir name]} {
        } elseif {[regexp {^\s+wire\s+(input|output|inout)\s+\d+\s+\\(\S+)} $line -> dir name]} {
        } else { continue }
...
proc report_port_connection {{outfile ""}} {
    ...
    foreach line [split $dump "\n"] {
        if {[regexp {^\s+wire\s+width\s+(\d+)\s+(input|output|inout)\s+\d+\s+\\(\S+)} $line -> w d n]} {
            lappend port_list $n ; set port_dir($n) $d ; set port_width($n) $w
        } elseif {[regexp {^\s+wire\s+(input|output|inout)\s+\d+\s+\\(\S+)} $line -> d n]} {
